...
m=101; n=101; % includes boundary nodes, mesh spacing 1/(m-1) and 1/(n-1)
...
%K=spalloc(N,N,numel(t)); % zero matrix in sparse format: zeros(N) would be "dense"
...
contourf(x,y,reshape(U,size(x)),51,'LineStyle','none')
axis square
end

function [K,F] = buildKF(N,T,t,p)
K=spalloc(N,N,numel(t)); % zero matrix in sparse format: zeros(N) would be "dense"
%K=sparse(N,N);
F=zeros(N,1); % load vector F to hold integrals of phi's times load f(x,y)
