...
%   we include some padding in here
    leftEdge = min(x);  % max(meanX-stdX*stdTimes,min(x));
    riteEdge = max(x);  % min(meanX+stdX*stdTimes,max(x));
else % stdV==0, wow, all your data points are equal
    leftEdge=min(x)-1000*eps; % padd it by 1000, seems reasonable
...
if stdY>(10*eps) % just checking there are more than two different points to the data, checking for rounding errors.
    botEdge = min(y);  % max(meanY-stdY*stdTimes,min(y));
    topEdge = max(y);  % min(meanY+stdY*stdTimes,max(y));
else % stdV==0, wow, all your data points are equal
    botEdge=min(y)-1000*eps; % padd it by 1000, seems reasonable
